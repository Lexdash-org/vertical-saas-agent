 ...
 *   npx tsx scripts/extract-team-members/run-one.ts example.com [--json]   (--json = print ONLY the array)
 *
...
const input = args[0];
if (!input) {
  console.error('usage: npx tsx scripts/extract-team-members/run-one.ts <website> [--json]');
  process.exit(1);
}
